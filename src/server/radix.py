...
class Radix(threading.Thread):
	...
	def __init__(self,url,downloadDir=None):
		...
		self.output_file = downloadDir + "/%(title)s-%(id)s.%(ext)s"
		self._url=url
		self.play_list = False
		self.entries = None
		ydl_opts = {
    		'quiet':True,
    		'verbose':False,
		...
		with youtube_dl.YoutubeDL(ydl_opts) as ydl:
			...
			#ydl.add_metadata_hook(self.meta_hook)
			...
			if 'title' in self._info.keys():
				...
				self.name = url
			if 'entries' in self._info.keys():
				self.play_list = True
				self.entries = self._info['entries']
